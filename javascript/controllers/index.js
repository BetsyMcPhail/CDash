...
.controller('IndexController', function IndexController($scope, $rootScope, $http) {
  ...
  $http({
    ...
    for (var i = 0, len = cdash.buildgroups.length; i < len; i++) {
...
.directive(
  "buildGroup", function() {
    ...
    return({
      link: link,
      restrict: "A",
      templateUrl: "views/partials/buildgroupTable.html"
    });

    // I bind the JavaScript events to the scope.
    function link( scope, element, attributes ) {
      console.log( "buildGroup directive linking." );
    }
  }
);
