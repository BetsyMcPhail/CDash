...
function echo_main_dashboard_JSON($project_instance, $date)
{
  ...
  while($buildgroup_array = pdo_fetch_array($buildgroup_result))
    ...
    $buildgroup_response['buildduration'] = 0;
  ...
  foreach($build_rows as $build_array)
    ...
    for ($j = 0; $j < count($buildgroups_response); $j++)
      {
      if ($buildgroups_response[$j]['name'] == $groupname) // TODO: change to id
        {
        $i = $j;
        break;
        }
    ...
    else if(strtolower(substr($build_array["osname"],0,3)) == 'gnu')
      {
      $buildplatform='gnu';
      }

    if(isset($_GET["parentid"]) && empty($site_response))
      {
    ...
    if(!empty($labels_array))
      {
      $build_response['labels'] = array();
      foreach($labels_array as $label)
        {
        $build_response['labels'][] = $label;
        }
      }
    ...
    if($build_array['countbuilderrors']>=0)
      ...
      $buildgroups_response[$i]['buildduration'] += $duration;
    ...
    while($coverage_array = pdo_fetch_array($coverages))
      ...
      if ($build_array["subprojectgroup"])
        ...
        if (array_key_exists($groupId, $subproject_groups))
          ...
          $coverage_response['group'] = $groupId;
          }
        }

      $coverage_response['percentage'] = $percent;
      $coverage_response['percentagegreen'] = $coverageThreshold;
      $coverage_response['percentageyellow'] = $coverageThreshold * 0.7;
      $coverage_response['fail'] = $coverage_array["locuntested"];
      $coverage_response['pass'] = $coverage_array["loctested"];
      ...
      if(pdo_num_rows($coveragediff) >0)
        ...
        $coverage_response['faildiff'] = $locuntesteddiff;
        $coverage_response['passdiff'] = $loctesteddiff;
        }
      ...
      if(!empty($labels_array))
        {
        $coverage['labels'] = array();
        foreach($labels_array as $label)
          {
          $coverage['labels'][] = $label;
          }
        }
    ...
    while($dynanalysis_array = pdo_fetch_array($dynanalysis))
      ...
      if(!empty($labels_array))
        {
        $DA_response['labels'] = array();
        foreach($labels_array as $label)
          {
          $DA_response['labels'][] = $label;
          }
        }
  ...
  if (!empty($subproject_groups))
    {
    $response['subprojectgroups'] = array();
    foreach ($subproject_groups as $groupid => $group)
      ...
      $subprojectgroup_response = array();
      $subprojectgroup_response['name'] = $group->GetName();
      $subprojectgroup_response['id'] = $group->GetId();
      $subprojectgroup_response['thresholdgreen'] =
        $group->GetCoverageThreshold();
      $subprojectgroup_response['thresholdyellow'] =
        $group->GetCoverageThreshold() * 0.7;
      $subprojectgroup_response['coverage'] = $coverage;
      $subprojectgroup_response['tested'] = $tested;
      $subprojectgroup_response['untested'] = $untested;
      $response['subprojectgroups'][] = $subprojectgroup_response;
      }
    }
...
function get_child_builds_hyperlink($parentid, $filterdata)
{
  ...
  // Replace index.php with viewChildren.php
  $baseurl = str_replace("index.php", "viewChildren.php", $baseurl);
